...
for i in range(0,100):
    ...
    model.add(layers.Conv2D(128, 3, activation='tanh', padding='same'))
    model.add(layers.AveragePooling2D(pool_size=(2, 2), strides=(2, 2)))
    ...
    logger.info("Performance of network:")
    logger.info("Accuracy on training data:   {:6.2f}%".format(history.history['accuracy'][-1]*100))
    logger.info("Accuracy on validation data: {:6.2f}%".format(history.history['val_accuracy'][-1]*100))
    logger.info("Accuracy on testing data:    {:6.2f}%".format(test_acc*100))

    # plot_confusion_matrix(model, test_images, categorical_test_labels)
